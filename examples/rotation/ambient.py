...
# Permutations of the ambient space.
perms = [
    jnp.array((2, 0, 3, 4, 1)),
    jnp.array((1, 4, 0, 3, 2)),
    jnp.array((3, 4, 1, 0, 2)),
...
def forward(params: Sequence[jnp.ndarray], fns: Sequence[Callable], x:
    ...
    y = permute.forward(y, perms[0])
    y = realnvp.forward(y, 2, params[1], fns[1])
    y = permute.forward(y, perms[1])
    y = realnvp.forward(y, 2, params[2], fns[2])
    y = permute.forward(y, perms[2])
    y = realnvp.forward(y, 2, params[3], fns[3])
    y = permute.forward(y, perms[3])
...
def sample_ambient(rng: jnp.ndarray, num_samples: int, bij_params:
    ...
    those samples to the Cartesian product of the sphere and circle.
    ...
    Returns:
        z, zaxis, zangle: A tuple containing the ambient samples and the
            projection of the samples to the Cartesian product of the sphere and
            circle.

    """
    z = random.normal(rng, [num_samples, 5])
    z = forward(bij_params, bij_fns, z)
    zaxis, zangle = z[..., :3], z[..., 3:]
    zaxis /= jnp.linalg.norm(zaxis, axis=-1)[..., jnp.newaxis]
    zangle /= jnp.linalg.norm(zangle, axis=-1)[..., jnp.newaxis]
    return z, zaxis, zangle
