...
def manifold_ode_log_prob(params: List, rng: random.PRNGKey, num_samples: int) -> Tuple[jnp.ndarray]:
    ...
    x = sample_uniform(rng_x, [num_samples, 3])
...
def main():
    ...
    del w, Z, log_approx, approx, log_target, target
    print('manode - Mean MSE: {:.5f} - Covariance MSE: {:.5f} - KL$(q\Vert p)$ = {:.5f} - KL$(p\Vert q)$ = {:.5f} - Rel. ESS: {:.2f}%'.format(mean_mse, cov_mse, klqp, klpq, ress))
    ...
    # obs, log_prob = manifold_ode_log_prob(params, rng_sample, 2000000)
    # prob = jnp.exp(log_prob)
    # base = project_to_sphere(random.normal(rng_basis, [3]))
