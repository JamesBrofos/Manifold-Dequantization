...
parser.add_argument('--num-hidden', type=int, default=10, help='Number of hidden units to use in RealNVP if this is the ambient density selected.')
...
def ambient_gaussian_mixture(params, x):
    ...
    d += 0.7 * spst.multivariate_normal.pdf(x, -mu, Sigma)
    return d

def density(y, ambient, params):
    ...
    """
    radii = np.linspace(0, 5., args.num_discrete + 1)[1:]
...
def kl(params, rng):
    ...
    y = project(x)
    logq = np.log(density(y, nvp.ambient_nvp_chain_density, params))
    logp = np.log(density(y, ambient_gaussian_mixture, params_actual))
...
params, kl = ambient_flow(params, kl)
